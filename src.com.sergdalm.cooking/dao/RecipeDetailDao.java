...
public class RecipeDetailDao implements Dao<RecipeDetail> {
    ...
    public RecipeDetail save(RecipeDetail recipeDetail) {
        try (var preparedStatement = ConnectionManager.getPreparedStatement(SAVE)) {
            ...
            ValueSetterUtil.setPreparedStatementValueIfNotNull(preparedStatement, 4, recipeDetail, RecipeDetail::getActiveCookingTimeMinute, Types.INTEGER);
            ValueSetterUtil.setPreparedStatementValueIfNotNull(preparedStatement, 5, recipeDetail, RecipeDetail::getTotalCookingTimeMinute, Types.INTEGER);
            ValueSetterUtil.setPreparedStatementValueIfNotNull(preparedStatement, 6, recipeDetail, RecipeDetail::getNumberOfPortions, Types.INTEGER);
    ...
    public RecipeDetail get(int id) {
        try (var preparedStatement = ConnectionManager.getPreparedStatement(GET)) {
            ...
            resultSet.next();
            return buildRecipeDetail(resultSet);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public List<RecipeDetail> getAll() {
        try (var preparedStatement = ConnectionManager.getPreparedStatement(GET_ALL)) {
            ...
            while (resultSet.next()) {
                recipeDetails.add(buildRecipeDetail(resultSet));
            }
    ...
    // ВОПРОС: ничего что я здесь использую строковые литералы?
    // Или лучше вынести их в статические финальные поля?
    private RecipeDetail buildRecipeDetail(ResultSet resultSet) {
        try {
    ...
    public RecipeDetail update(RecipeDetail recipeDetail) {
        try (var preparedStatement = ConnectionManager.getPreparedStatement(UPDATE)) {
            ...
            ValueSetterUtil.setPreparedStatementValueIfNotNull(preparedStatement, 2, recipeDetail, RecipeDetail::getActiveCookingTimeMinute, Types.INTEGER);
            ValueSetterUtil.setPreparedStatementValueIfNotNull(preparedStatement, 3, recipeDetail, RecipeDetail::getTotalCookingTimeMinute, Types.INTEGER);
            ValueSetterUtil.setPreparedStatementValueIfNotNull(preparedStatement, 4, recipeDetail, RecipeDetail::getNumberOfPortions, Types.INTEGER);
